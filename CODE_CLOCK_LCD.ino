#include <Wire.h>
#include <LiquidCrystal_I2C.h> // Thư viện i2c cho lcd nhấn vào link để down: https://github.com/Vothongminh/LIBRARY-VTM/blob/main/LiquidCrystal_I2C-1.1.2.zip
...

{
  ...
  if (digitalRead(10) && state1 == 1) { //  tăng giờ hoặc phút
    dg = 1;
    state1 = 0;

  } else if (!digitalRead(10) && state1 == 0) {
    state1 = 1;
  }

  if (digitalRead(9) && state2 == 1) { // // giảm giờ hoặc phút
    dg = -1;
    state2 = 0;
  } else if (!digitalRead(9)&state2 == 0) {
    state2 = 1;
  }
  switch (cnt) {
    case 1:
